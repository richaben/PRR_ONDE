...
```{r include=FALSE}
masque_metropole <- depts %>% 
  filter(code_insee %ni% conf_dep) %>% 
  filter(code_insee %ni% c(971:976)) %>% 
  summarise() %>% 
...
### **Conditions d'écoulement lors des campagnes usuelles (mai à septembre `r unique(df_categ_obs_3mod$Annee)`)** {.tabset}

```{r}
plot_bilan_prop <- function(data_bilan, lib_ecoulement) {
  data_bilan %>% 
    ggplot() +
    aes(
      y = frq, 
...
#####-----------
### plot 3 modalites
plot_bilan_prop(df_categ_obs_3mod, lib_ecoulement = lib_ecoul3mod)
```

#### Typologie départementale

```{r fig.height = 5.5, fig.width = 8, align = "center", warning = FALSE}
plot_bilan_prop(df_categ_obs_4mod, lib_ecoulement = lib_ecoul4mod)
```
...
plot_assecs_consecutifs(duree_assecs_df)
```

## Bilans départementaux {.tabset}

```{r, results='asis'}
for (d in depts_sel$code_insee) {
